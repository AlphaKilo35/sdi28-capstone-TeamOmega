...
function NavBarComponent() {
  ...
  const handleLogout = () => {
    ...
    .then((res)=>res.json())
    .then((data)=> data ? navigate('/login') : console.error('Server Error'))
  }
        ...
        <div className="absolute ml-20 flex justify-between w-full">
          ...
          <div className=" absolute right-28 flex ">
            ...
            <div className="relative">
              ...
              <div
                className={`left-3 top-8 transform transition-all duration-300 ease-out absolute rounded-md p-2 text-sm border border-gold-400 bg-black ${
                  dropdownSelect
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 invisable translate-y-[-20px] pointer-events-none"
                }`}
              >
                <div className="p-1 h-full cursor-pointer" onClick={()=>navigate('/profiles')}>Profile</div>
                <div className="p-1 h-full text-nowrap cursor-pointer" onClick={handleLogout}>Log out</div>
              </div>
            </div>
          </div>
        </div>
