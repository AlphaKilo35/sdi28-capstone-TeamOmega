...
app.post('/flights/:id', (req, res) => {
  ...
  knex('flight_table').returning('*').insert({id, airframe, number_pax, dropzone, departure_area, date, time})
  .then(data => {
...
app.get('/flights', (req,res)=>{
  knex('flight_table').select('*')
  .then(res => res.json())
  .then(data => console.log(data))
})
//Update entries
app.patch('/flights/:id', (req, res) => {
  knex('flight_table').where('id', req.params.id).update(req.body).returning('*')
  .then(data => {
    res.json(data)
  })
})
